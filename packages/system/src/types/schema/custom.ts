/**
 * The `ListSchema` type defines the structure and behavior of a multi-step form, enabling
 * dynamic and type-safe handling of each step. It accepts the following generic parameters:
 *
 * - `Render`: Specifies the type of the rendered output for each form step. This can vary
 *   depending on the framework; for example, in React, it would typically be `ReactNode`.
 *
 * - `Values`: A type extending `ListValues` that defines the structure of the multi-step form,
 *   including the values handled in each phase.
 *
 * - `Inputs`: An object type representing additional values available during form execution,
 *   beyond those generated by the multi-step form itself.
 *
 * - `Params`: An object type defining the values accessible when rendering each form step
 *   in the multi-step process.
 *
 * This `ListSchema` type is a recursive type that relies on all these other types to define the
 * structure and behavior of a multi-step form.
 */
